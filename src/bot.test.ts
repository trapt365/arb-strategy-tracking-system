...
describe('bot — профиль клиента: обязательный первый шаг (Story 9.1)', () => {
  ...
  it('AC1: после A1.2 — offer «Добавить участников / Дальше»; «Дальше» → существующий flow сбора (Story 10.2)', async () => {
    const { bot, calls } = buildProfileBot();
    await bot.handleUpdate(commandUpdate('/newclient'));
    ...
    expect(offerButtons).toContain('Добавить участников');
  ...
  it('топы A3.2: LLM экстракция вызвана, топ сразу добавляется без ретрая (Story 11.5)', async () => {
    ...
    await bot.handleUpdate(plainTextUpdate('Ромашка'));
    await bot.handleUpdate(plainTextUpdate('Продаём ромашки бизнесу'));
    // A3.2 — в расширенном блоке, нажимаем «Добавить участников».
    ...
    const added = texts91(calls, before).find((t) => t.includes('Участник добавлен'));
  ...
  it('расширенный: /skip и «не знаю» пропускают вопрос без заполнения поля (Story 10.2)', async () => {
    ...
    let before = calls.length;
    await bot.handleUpdate(callbackUpdate('f0p_ext'));
    // Первый расширенный вопрос — A3.2 (топы), прогресс (1/15).
    expect(texts91(calls, before).some((t) => t.includes('(1/15)'))).toBe(true);
    expect(texts91(calls, before).some((t) => t.includes('участников'))).toBe(true);

    // Пропускаем A3.2 (топы) — /skip на type=tops обрабатывается как обычный пропуск.
    before = calls.length;
    await bot.handleUpdate(commandUpdate('/skip'));
    // A1.3 история (2/15) — a3_3 удалён.
    expect(texts91(calls, before).some((t) => t.includes('(2/15)'))).toBe(true);

    before = calls.length;
    await bot.handleUpdate(commandUpdate('/skip')); // пропуск A1.3
    // A1.4 владельцы (3/15).
    expect(texts91(calls, before).some((t) => t.includes('(3/15)'))).toBe(true);

    before = calls.length;
    await bot.handleUpdate(plainTextUpdate('не знаю')); // ответ A1.4
    expect(texts91(calls, before).some((t) => t.includes('(4/15)'))).toBe(true);
    ...
    expect(after.some((t) => t.includes('(10/15)'))).toBe(true);
  });
  ...
  it('числовые A2: один переспрос, повтор принимается (мягкая валидация 8.6, Story 10.2)', async () => {
    ...
    await bot.handleUpdate(callbackUpdate('f0p_ext'));
    await bot.handleUpdate(commandUpdate('/skip')); // A3.2 (1/15) → skip → A1.3
    await bot.handleUpdate(commandUpdate('/skip')); // A1.3 (2/15) → skip → A1.4
    await bot.handleUpdate(commandUpdate('/skip')); // A1.4 (3/15) → skip → A2.1 выручка (4/15)

    let before = calls.length;
    await bot.handleUpdate(plainTextUpdate('точно не скажу')); // нет числа → переспрос
    expect(texts91(calls, before).some((t) => t.includes('Не вижу числа'))).toBe(true);
    expect(texts91(calls, before).some((t) => t.includes('(5/15)'))).toBe(false);

    before = calls.length;
    await bot.handleUpdate(plainTextUpdate('точно не скажу')); // второй раз → принять → A2.2 (5/15)
    expect(texts91(calls, before).some((t) => t.includes('(5/15)'))).toBe(true);
  });
...
describe('bot — Story 10.2: /advanced команда', () => {
  ...
  it('(a) /advanced в онбординге (offer pending после A1.1 + A1.2) → начинает A3.2 как первый расширенный (1/15)', async () => {
    ...
    // Должен начать расширенный профиль с A3.2 (1/15) — первый вопрос расширенного.
    expect(after.some((t) => t.includes('(1/15)'))).toBe(true);
    expect(after.some((t) => t.includes('участников'))).toBe(true);
  });
  ...
  it('(b) /advanced без онбординга, active client → дозаполнение профиля (reply содержит «Дозаполняем»)', async () => {
    ...
    // Первый расширенный вопрос — A3.2 (1/15), т.к. tops не заполнены.
    expect(after.some((t) => t.includes('(1/15)'))).toBe(true);
  });
  ...
  it('(f) /advanced когда уже в расширенном режиме (profileExtended=true) → «уже дополняется» + повтор вопроса', async () => {
    ...
    await bot.handleUpdate(commandUpdate('/advanced'));
    const after = textsAdv(calls, before);
    // Бот сообщает что профиль уже дополняется и показывает текущий вопрос (A3.2 = 1/15).
    expect(after.some((t) => t.includes('уже') || t.includes('дополняется') || t.includes('продолжай'))).toBe(true);
    expect(after.some((t) => t.includes('(1/15)') || t.includes('участников'))).toBe(true);
  });
...
describe('bot — «➕ Дозаполнить профиль» из карточки клиента (Story 9.1)', () => {
  ...
  it('AC3: карточка показывает профиль и кнопку дозаполнения; ответы дописываются в card.json', async () => {
    ...
    // Профиль в карточке: суть, топы, счётчик расширенной части.
    expect(cardMsg.payload.text).toContain('Суть: Продаём ромашки бизнесу');
    // Story 11.9: DM удалён → строки «DM:» нет.
    // Story 10.2: tops теперь в расширенном блоке → карточка с tops = 1/15.
    expect(cardMsg.payload.text).toContain('расширенный 1/15');
    ...
    // Story 11.9: a3_3 удалён; a3_2 первый в расширенном, но карточка уже содержит
    // tops → пропускается как отвеченный; первый показанный — A1.3 (2/15).
    expect(after.some((t) => t.includes('(2/15)'))).toBe(true);
...
describe('bot — Story 9.5: вопросник с голосовыми ответами', () => {
  ...
  it('(j) f0q_owner кнопкой → owner записывается, переходит к B5.1 (hypo_collect)', async () => {
    ...
    await bot.handleUpdate(callbackUpdate('f0p_ext')); // → расширенный (A3.2 = 1/15)
...
describe('bot — Story 11.5: LLM-экстракция участника A3.2', () => {
  ...
  it('(a) LLM fallback: extractTopWithLlm возвращает topFromRawAnswer-образный объект — топ добавлен', async () => {
    ...
    const added = texts115(calls, before).find((t) => t.includes('Участник добавлен'));
    expect(added).toBeDefined();
  ...
  it('(c) после f0p_top_done показан вопрос A1.3', async () => {
    ...
    const added = texts115(calls, before).find((t) => t.includes('Участник добавлен'));
    expect(added).toBeDefined();
    // После добавления топа — нажать «Готово» → следующий вопрос A1.3.
    const b2 = calls.length;
    await bot.handleUpdate(callbackUpdate('f0p_top_done'));
    // A1.3: «Год основания» или «история»
    expect(texts115(calls, b2).some((t) => /история|Год основания/i.test(t))).toBe(true);
  });
...
describe('bot — Story 11.8: пакетный приём команды A3.2', () => {
  ...
  it('(c) f0p_batch_ok → все tops добавлены → переход к A1.3', async () => {
    const extractAllTopsWithLlm = vi.fn().mockResolvedValue([
      { name: 'Иван', title: 'CEO', authority: null, area: null },
    ]);
    const { bot, calls } = buildBot({
    ...
    // Next question should be A1.3 («история» or «Год основания»)
    expect(sentTexts.some((t) => /история|Год основания/i.test(t))).toBe(true);
  });
