...
**Given** полировка шапки трекера гипотез (10.8) по golden-eval 10.07
**When** косметика применена к F5-трекеру
**Then** в шапке `full`-отчёта добавлены строки «Подготовил: {имя трекера}» и «База: снимок нед.{N-1} ({M} гипотез)»; статусы в ячейках с заглавной («🟢 Работает», не «🟢 работает»); карта `STATUS_EMOJI` дополнена вариантами «завершена→🟢», «приостановлена/⏸️→⛔», «идёт/in progress→🟡» (неизвестный статус по-прежнему `⬜`); строка «Период» — диапазон недель «Недели 1–{N}», если известен, иначе даты встреч

**Given** регресс
**When** косметика завершена
**Then** ни одна пользовательская выдача не длиннее ~15 строк без ссылки; весь регресс зелёный

### Story 10.8: [обогащение] Трекер гипотез — структура уровня geonline

As a **коуч практики (Азиза)**,
I want **трекер гипотез, структурно совпадающий с моим ручным эталоном (geonline нед.12)**,
So that **клиент получает документ уровня трекера, а не плоский список статусов**.

> Golden-eval (10.07) показал: 10.5 построил F5 плоским (список изменений + счётчик + выводы) — по своей спеке с «компактным Телеграмом». Эта история заменяет формат на структурный. Спека: `spec-10-8-treker-gipotez-struktura-geonline.md`. Эталон: `Vault1/geonline трекер гипотез неделя 12.md`. **Должна идти ДО 10.6 golden-eval.**

**Acceptance Criteria:**

**Given** лист гипотез (статусы + дельта) и F1-отчёты встреч недели
**When** трекер запрашивает трекер гипотез
**Then** отчёт содержит в порядке: шапку (период/подготовил/для/база) · легенду 7 статусов · секции по департаментам (ответственный+метрики, таблица «Обновления нед.N-1→N» с колонками `#|Гипотеза|Статус N-1|Статус N|Комментарий`, таблица «🆕 Новые» с колонками `#|Гипотеза|Статус|Запуск|Результат|Следующий шаг`) · сводную матрицу по департаментам · ключевые выводы · методологическую сноску. Структура сверяется с эталоном — это ЖЁСТКИЙ критерий, НЕ плоский формат 10.5

**Given** богатые ячейки (комментарий/запуск/результат/следующий шаг/прямая речь) не заполняются трекером вручную
**When** отчёт строится
**Then** бот синтезирует их LLM из F1-отчётов недели (тот же источник, что 9.7); департамент/ответственный/метрики — из OKR-листа/профиля; при отсутствии встреч — структура строится из листа+дельты без комментариев (не блокировать)

**Given** богатый трекер не влезает в сообщение Телеграма
**When** отчёт доставляется
**Then** полный трекер уходит вложением-файлом (`.md`, `sendDocument`); в чат — компактная сводка (матрица по департаментам + топ-3 сигнала + «полный во вложении»); НЕ обрезать через splitForTelegram

**Given** переиспользование и регресс
**When** обогащение внедрено
**Then** `computeDelta`/снимок недели переиспользованы (старый снимок не роняет пайплайн); старый плоский форматтер сохранён как `formatHypoReportFlat`; `clientId==='geonline'` fallback цел; инварианты Epic 7 целы; `tsc`+`vitest`+canary зелёные

## Epic 11: Живой прогон WP-39 — устойчивость ядра на свежем клиенте (13 июля)

> Добавлен 2026-07-13 (correct-course по живому прогону бота пилотом; `correct-course-tz-2026-07-13-live-run.md` — 12 находок с местами в коде и симптомами). Формат stories — лёгкий (solo MVP): 3 AC, ~5 задач, 1 проход ревью. **Порядок: сначала 3 блокера ядра (11.1 → 11.2 → 11.3), затем остальное (11.4 → 11.5 → 11.6 → 11.7 → 11.8 → 11.9).** Первоисточник — тот же ТЗ живого прогона.

**Дрифт (признан):** онбординг end-to-end подтверждён рабочим (приём стратегии → извлечение 7 целей/39 KR/32 гипотезы/8 участников → создание таблицы → регистрация `geonline`), но **ни один из трёх отчётов ядра не собрался на свежем клиенте**: бот упал на аудио >20 МБ (нет `bot.catch` → процесс убит, pm2 рестарт), read-пайплайны словили 403 на новых клиентских таблицах (read/write identity разъезд), а после починки прав отчёт по встрече упал на ZodError в `parseOkrs` из-за одной пустой ячейки `owner`. Три блокера ядра держат «рабочую версию, которую не стыдно показать» (условие оплаты Айдара).

**Цель:** чистый повторный прогон трёх отчётов на свежем клиенте — любая ошибка в хендлере не роняет прод; аудио >20 МБ деградирует graceful; read-пайплайны (F1 отчёт, F5 трекер) читают новые клиентские таблицы без ручной расшивки; одна пустая ячейка не роняет весь отчёт. Плюс — снять острые UX/надёжность-занозы прогона (ложные советы при ошибке API, строгий парсер участника, полотно черновика, приём готового транскрипта) и полировка (термины, security, ребрендинг).

**FRs (новые):**
- FR119 [MVP, блокер]: глобальный `bot.catch` (любая ошибка в хендлере логируется и не убивает процесс) + graceful-обработка аудио/файла >20 МБ (проверка `file_size` до `getFile`; при превышении — понятное сообщение клиенту, не краш).
- FR120 [MVP, блокер]: устойчивый `parseOkrs` — пустой/битый `owner` (и прочие необязательные поля строки) не роняют весь отчёт (skip/default «—» + warn, не throw); онбординг проставляет owner-заглушку для дозаполнения, а не пустую ячейку.
- FR121 [MVP, блокер]: единая read/write identity для клиентских таблиц — таблица, созданная под user-OAuth, расшаривается на email сервис-аккаунта (editor) при создании (или все read-операции ведутся под тем же OAuth, что и write); все read-пайплайны (F1 отчёт `_okr`/`_stakeholder_map`/`_f5_metrics`, F5 трекер `_hypotheses`) работают на новом клиенте без ручной расшивки.
- FR122 [MVP]: классификация ошибок Claude API в пользовательских сообщениях — ветвление по `httpStatus`/`anthropicErrorType` (billing/credit vs rate-limit vs too-large-context); совет «убери файлы» только когда контекст реально велик, не при единственном файле.
- FR123 [MVP]: разбор участника профиля (A3) через LLM-экстракцию `{имя, должность, полномочия, зона}` из естественной фразы (как в приёме стратегии), недостающие поля — точечный доспрос, а не отказ всего ответа хард-ретраем.
- FR124 [MVP]: компактная доставка черновика онбординга — резюме (счётчики целей/KR/гипотез/участников + 1–2 строки «что дозаполнить») + гиперссылка на Google Sheet; полный список неполных KR — в таблице, не полотном в чат.
- FR125 [MVP]: `/report` принимает готовый транскрипт (md/txt/docx) как вход наравне с аудио/ссылкой; детект «транскрипт встречи» vs «документ онбординга», чтобы md-транскрипт при активном клиенте не перехватывался флоу онбординга.
- FR126 [рост/фича]: пакетный приём и front-load онбординга — детект типа поданного входа (фото/OCR, аудио, документ xlsx/docx/pdf) → маршрутизация в нужный парсер; карта вопросов в начале + пакетный сбор/загрузка документов + предзаполнение + review-режим (принять/править) вместо строгого Q&A.
- FR127 [полировка]: термин «топы» → «участники работы с трекером» по всему онбордингу; убрать вопрос про decision maker (`a3_3`); редакция bot-токена из логируемого ctx (security); ребрендинг @username в BotFather (Manual TODO, `geonline_report_bot` → нейтральный).

**Инварианты Epic 7 (1–4) без изменений.** Регресс-guardrail всего эпика: `clientId === 'geonline'` fallback, `GEONLINE_F0_SHEET_ID`, полный F1-цикл Geonline (canary + golden + весь vitest + tsc) зелёные после каждой story; боевая таблица Geonline не затрагивается. **PENDING Тимур (не код):** пополнить баланс Anthropic API (на прогоне =0, ядро не протестировать вживую до пополнения; учесть COGS LLM в ценообразовании ~50k тг/мес); BotFather-ребрендинг username (FR127).

### Story 11.1: [P0, блокер] Глобальный `bot.catch` + graceful-обработка аудио >20 МБ

As a **коуч практики (Азиза)**,
I want **чтобы большой файл или любая ошибка в хендлере не роняли бот целиком**,
So that **прод не падает и не перезапускается pm2 из-за одной записи встречи**.

**Acceptance Criteria:**

**Given** пилот загрузил `жанель 11.m4a` (~27 МБ) — `getFile` вернул «400: file is too big» (лимит Bot API = 20 МБ), ошибка не поймана, процесс упал («No error handler was set!»), pm2 перезапустил
**When** любой хендлер бросает исключение
**Then** установлен глобальный `bot.catch` — ошибка логируется (с сохранением диагностики), процесс НЕ падает, клиент получает понятное сообщение, а не тишину/краш

**Given** входящий аудио/видео/файл встречи размером >20 МБ (`handleMeetingFileIntake`, [bot.js:3535](~/apps/strategy-tracking-system/dist/bot.js#L3535))
**When** файл поступает в чат
**Then** бот проверяет `file_size` до `getFile` и при >20 МБ деградирует graceful — сообщает клиенту «файл великоват, сожми/разбей» (или альтернативный путь загрузки), не вызывает падающий `getFile`

**Given** мультиклиентность и регресс Geonline
**When** обработчик ошибок и проверка размера внедрены
**Then** файлы ≤20 МБ обрабатываются как раньше; canary + golden + весь vitest + tsc зелёные; боевая таблица Geonline не затрагивается

### Story 11.2: [P0, блокер] Устойчивый парсер отчёта — пустой `owner` не роняет весь пайплайн

As a **коуч практики (Азиза)**,
I want **чтобы одна незаполненная ячейка в листе `_okr` не роняла весь отчёт**,
So that **отчёт по встрече и недельный собираются поверх пробелов свежего клиента**.

**Acceptance Criteria:**

**Given** лист `_okr` со строками OKR (напр. 18/19/27/36) с пустым `owner` — сейчас `parseOkrs` бросает ZodError (схема требует ≥1 символ), одна пустая ячейка роняет **весь** отчёт (`sheets:invalid_value`); [sheets.js:486 parseOkrs](~/apps/strategy-tracking-system/dist/adapters/sheets.js#L486) → `readClientContext` → `processJob`
**When** отчёт строится поверх листа с пустыми `owner`
**Then** парсер терпит пустой/битый `owner` (дефолт «—» / warn, не throw); отчёт собирается, неполные строки помечены, а не роняют пайплайн

**Given** онбординг создаёт KR без владельца
**When** таблица клиента наполняется
**Then** онбординг проставляет owner-заглушку (для дозаполнения), а не пустую ячейку — у свежего клиента пробелы норма, отчёт должен собираться поверх них

**Given** регресс Geonline и golden
**When** устойчивый парсер внедрён
**Then** валидные строки парсятся как раньше; canary + golden + весь vitest + tsc зелёные; боевая таблица Geonline не затрагивается

### Story 11.3: [P1, блокер] Единая read/write identity — клиентские таблицы расшарены на сервис-аккаунт

As a **коуч практики (Азиза)**,
I want **чтобы бот читал таблицу нового клиента без ручной расшивки прав**,
So that **все три отчёта работают на свежем клиенте, а не только на Geonline**.

**Acceptance Criteria:**

**Given** клиентская таблица создаётся под user-OAuth (обход квоты сервис-аккаунта), а read-пайплайны читают под сервис-аккаунтом `geonline-tracking-bot@geonline-report-bot.iam.gserviceaccount.com` — сейчас 403 `forbidden_or_revoked` блокирует F5 трекер (`readHypothesesSheet`, [sheets.js:450](~/apps/strategy-tracking-system/dist/adapters/sheets.js#L450)) и встреча→отчёт/недельный (`_okr`/`_stakeholder_map`/`_f5_metrics`)
**When** таблица нового клиента создаётся при онбординге
**Then** она автоматически расшаривается на email сервис-аккаунта (editor) — либо все read-операции ведутся под тем же OAuth, что и write; read/write identity едина

**Given** свежий клиент (не Geonline) и любой read-пайплайн (F1 отчёт, F5 трекер, недельный)
**When** бот читает клиентскую таблицу
**Then** чтение проходит без 403 и без ручной расшивки; бот больше не уходит в «Задержка, пиши вручную» из-за прав

**Given** регресс Geonline и мультиклиентность
**When** расшивка/единая identity внедрены
**Then** Geonline (уже расшарен) не затронут; клиенты не смешиваются; canary + golden + весь vitest + tsc зелёные

### Story 11.4: [P1] Классификация ошибок Claude API — честное сообщение вместо «убери файлы»

As a **коуч практики (Азиза)**,
I want **правдивое сообщение при ошибке сборки черновика**,
So that **я не удаляю единственный файл по ложному совету, когда проблема в кредитах API**.

**Acceptance Criteria:**

**Given** сборка черновика упала с `400 invalid_request_error: "Your credit balance is too low"` при единственном файле 116 КБ — сейчас `f0.draft_failed` всегда советует «убери лишние файлы, собери снова: /draft»; `runF0FullDraft` f0-onboarding.js:80 → `buildF0Draft` bot.js:2380, классификация в claude.js:209
**When** обработчик ловит ошибку claude_api
**Then** ветвление по `httpStatus`/`anthropicErrorType`: 400 credit balance → «сервис временно недоступен (закончились кредиты API), напиши администратору»; rate-limit → «слишком много запросов, повтори позже»; too-large-context → совет про файлы

**Given** единственный поданный файл
**When** ошибка не про размер контекста
**Then** бот НЕ советует убирать файлы (совет «убери лишние» только когда файлов много / контекст реально велик)

**Given** регресс
**When** классификация внедрена
**Then** существующие ветки доставки/ошибок не сломаны; весь vitest + tsc зелёные

### Story 11.5: [P1] LLM-экстракция участника профиля (A3) вместо строгого формат-ретрая

As a **коуч практики (Азиза)**,
I want **надиктовать участника естественной фразой и чтобы бот разобрал поля сам**,
So that **корректно распознанный голос не отвергается из-за отсутствия разделителей**.

**Acceptance Criteria:**

**Given** голосом «Дамир, CEO, отвечает за стратегию и за управление» — ASR распознал верно (`f0.voice_ok`), но парсер полей ждёт жёсткий формат и выдал «Не разобрал ответ… Формат: Имя — должность, полномочия, зона» → `f0.profile_top_format_retry` (вопрос `a3_2`)
**When** участник вводится естественной фразой (голосом или текстом)
**Then** бот извлекает `{имя, должность, полномочия, зона}` LLM-экстракцией (как в приёме стратегии), а не хард-ретраем по разделителям

**Given** часть полей не извлеклась
**When** LLM-разбор дал неполный профиль участника
**Then** бот доспрашивает точечно только недостающее поле, а не отвергает весь ответ

**Given** регресс
**When** LLM-экстракция участника внедрена
**Then** ранее работавший форматный ввод по-прежнему принимается; весь vitest + tsc зелёные

### Story 11.6: [P2] Компактная доставка черновика онбординга — резюме вместо полотна

As a **коуч практики (Азиза)**,
I want **краткое резюме черновика со ссылкой на таблицу**,
So that **я вижу итог сборки, а не полотно из 20 неполных KR построчно**.

**Acceptance Criteria:**

**Given** после `/draft` бот вываливает полотно — перечисляет все неполные KR построчно в чат (`f0.draft_sent` / доставка черновика, bot.js после `runF0FullDraft`)
**When** черновик доставляется
**Then** в чат уходит компактное резюме: счётчики (напр. «7 целей · 39 KR, из них 20 без числовой базы · 32 гипотезы · 8 участников») + 1–2 строки «что дозаполнить» + гиперссылка на Google Sheet

**Given** полный список неполных KR
**When** черновик доставлен
**Then** детальная разбивка живёт в таблице (лист), а не в сообщении; пользовательская выдача компактна

**Given** регресс
**When** компактная доставка внедрена
**Then** ссылка на таблицу и счётчики корректны для любого клиента; весь vitest + tsc зелёные

### Story 11.7: [Фича/баг] `/report` принимает готовый транскрипт (md/txt/docx)

As a **коуч практики (Азиза)**,
I want **прислать готовую расшифровку встречи в отчёт, не гоняя через аудио**,
So that **имеющийся текстовый транскрипт сразу идёт в отчёт по встрече**.

**Acceptance Criteria:**

**Given** md-транскрипт встречи при активном клиенте — сейчас перехватывается флоу онбординга («Черновик уже собран, дозаполнение»), а не идёт в отчёт
**When** трекер присылает готовый транскрипт (md/txt/docx)
**Then** бот детектит «транскрипт встречи» vs «документ онбординга» и направляет транскрипт в `/report`-конвейер (существующий F1), минуя Soniox

**Given** активный клиент с уже собранным черновиком
**When** приходит транскрипт встречи
**Then** он не перехватывается онбордингом; отчёт по встрече строится и питает недельный (как аудио в 10.1)

**Given** регресс и мультиклиентность
**When** приём транскрипта внедрён
**Then** аудио/ссылка/онбординг-документы работают как раньше; клиенты не смешиваются; весь vitest + tsc зелёные

### Story 11.8: [Фича] Пакетный приём команды и front-load онбординга — детект типа + предзаполнение + review

As a **коуч практики (Азиза)**,
I want **загрузить список участников/ответы разом и править предзаполненное, а не отвечать по одному вопросу**,
So that **онбординг требует меньше усилий и клиент правит, а не вводит с нуля**.

**Acceptance Criteria:**

**Given** сейчас участники и профиль собираются строгим допросом по одному (текст/голос)
**When** трекер подаёт весь список разом (скриншот/фото → OCR, аудио, документ xlsx/docx/pdf)
**Then** бот сам определяет тип поданного входа (детект input-type → маршрутизация в нужный парсер) и разбирает список участников соответствующим образом

**Given** начало онбординга
**When** трекер входит в сбор профиля
**Then** бот показывает карту нужных вопросов и предлагает выбор: наговорить/заполнить сразу все ИЛИ загрузить документы (стратегия/презентация/оргструктура/финмодель), из которых извлекает ответы; онбординг продолжается с предзаполненными полями в review-режиме (принять/править), а не строгим Q&A

**Given** регресс и совместимость сессий
**When** пакетный приём и front-load внедрены
**Then** пошаговый путь остаётся доступен; поля профиля/сессии совместимы (optional в zod); весь vitest + tsc зелёные

### Story 11.9: [P2/полировка] Термины «участники», убрать decision-maker, security-редакция токена, ребрендинг username

As a **коуч практики (Азиза)**,
I want **чистые формулировки, короткий вопросник и отсутствие утечек в логах**,
So that **онбординг понятен клиенту, а логи безопасно шарить**.

**Acceptance Criteria:**

**Given** термин «топы» / «Добавить топов» / «А3. Люди и оргструктура» и вопрос про decision maker (`a3_3` «Кто принимает финальные решения»)
**When** тексты онбординга обновлены
**Then** «топы» → «участники работы с трекером» / «Внести список тех, кто будет работать с трекером» по всему онбордингу; вопрос `a3_3` убран из вопросника

**Given** в ctx-дампе ошибки bot-токен `8868172506:AAEN…` печатается в `out-0.log` (утечка при шаринге логов)
**When** ошибка логируется
**Then** bot-токен редактируется из логируемого ctx (не попадает в plaintext-логи)

**Given** @username бота в BotFather = `geonline_report_bot` (клиент видит geonline в ссылке), при том что display-имя уже «ARB Strategy Tracking Assistant Bot»
**When** полировка применена
**Then** оформлен Manual TODO Тимуру на BotFather-ребрендинг username (сменить через код нельзя); в коде/текстах geonline-строки не всплывают у не-geonline клиентов

**Given** регресс
**When** полировка завершена
**Then** ни одна пользовательская выдача не деградировала; `clientId==='geonline'` fallback цел; весь vitest + tsc зелёные
