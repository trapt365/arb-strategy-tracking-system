...
FR64: Команда `/approve <id>` для отправки клиенту
FR65: Команда `/edit <id>` — бот отправляет текст, трекер редактирует
FR66: Команда `/reject <id>` для отклонения и перегенерации
FR67: Команда `/status` для просмотра очереди
...
NFR31: Day 1: тест tldv на шумной записи и code-switching
...
NFR68: Day 1: тест транскрибации tldv worst case
NFR69: Проверка tldv API; fallback оценка Otter.ai, Fireflies.ai, AssemblyAI
...
- Поддержка tldv (primary) и Soniox (fallback), /upload для plain text fallback
...
UX-DR14: F5 последующие запросы: "📊 Name, metric за неделю?" + inline keyboard, Пт 17:00
...
UX-DR49: Unsure-content deletion: drop uncertain > mark with tags
...
FR27-FR32: Epic 1 — Tracker workflow (commands, approve, delivery)
...
**Go/No-Go gates:**
- Soniox WER < 15% на worst case ИЛИ `/upload` flow < 2 мин friction
...
**When** каждая обработана через Soniox API
**Then** WER < 15% на наихудший сценарий
**And** формат API ответа задокументирован
...
**Then** описаны: команды бота, approve/edit/reject flow, fallback при сбоях
...
### Story 1.2: Transcript Interface Contract и Soniox parser

As a **аналитик практики (Тимур)**,
I want **преобразовать транскрипт Soniox в единый JSON формат**,
So that **pipeline работает с любым провайдером через единый контракт**.

**Acceptance Criteria:**

**Given** JSON schema: `{speakers: [{name, segments: [{start, end, text}]}], metadata: {date, duration, meeting_type}}`
**When** транскрипт от Soniox получен через API
...
**And** confirmation: «✅ Отчёт отправлен {Name}.»
...
**Given** Айдар отправляет команду `/status` или нажимает [📊 Статус]
**When** запрос получен
