...
**Принцип уверенности контента:** AI отправляет только то, что может подкрепить данными. Неуверенные фрагменты без доказательства опускаются. Верификационные риски маркируются `[approximate]` и `[speaker_check]` в первые 2-3 недели калибровки доверия. Короткий уверенный отчёт лучше полного с оговорками.
...
- Timing: базовый ритм MVP — понедельник утро; перенос на пятницу рассматривать только если пилот покажет низкий response rate
...
**2. Soniox — AI transcription layer**
...
Что адаптируем: один вход → `/report <url>`; качественная русско-казахская транскрибация как основа для downstream анализа; готово к пересылке → copy-optimized F3-lite, секция «📱 для топа». Что не подходит: generic summary — у нас domain-specific анализ с OKR-привязкой.
...
| **Один вход → полный output** | Soniox | `/report <url>` → полный отчёт. Одно действие |
...
| **Готово к пересылке** | Soniox + Telegram flow | Copy-optimized F3-lite, секция «📱 для топа» |
...
- Один вход → полный output (Soniox) → `/report` как единственное действие
...
- Soniox transcript → domain-specific анализ с OKR-привязкой
...
**Trigger:** Cron Пн 8:00. Авто-деактивация после 3 нед молчания.

```mermaid
flowchart TD
    A["Cron Пн 8:00"] --> B{"Первый раз?"}
