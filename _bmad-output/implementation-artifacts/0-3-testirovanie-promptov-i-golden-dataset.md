...
- [x] Задача 3: Прогон F1-цепочки и итерация (КП: #1)
  - [x] 3.1 F1 на 7 транскриптах (≥5 ✅)
  - [x] 3.2 Ручная экспертная оценка Тимуром — **GO без правок 2026-04-20** (см. `docs/review-audio*.md`)
  - [x] 3.3 Доля правок: **0%** (порог < 50% ✅)
  - [x] 3.4 Итерации не требуются (попали в порог сразу). Промпты v1.0.0 в `prompts/CHANGELOG.md`
  - [x] 3.5 Стоп-сигнал не сработал

- [x] Задача 4: Тест F4-промпта (КП: #1)
  - [x] 4.1 F4 на 7 транскриптах (≥2 ✅). Багфикс Zod (`related_kr` null handling)
  - [x] 4.2 Оценка: 3 пункта повестки на каждый, ссылки на конкретные KR в `related_kr`, корректное использование `commitment` / `blind_zone` / `metric_discrepancy` источников

- [x] Задача 5: Golden dataset (КП: #1)
  - [x] 5.1 7 транскриптов в golden (1:1, группа 2-3 спикера, code-switching РУС↔КАЗ, разные департаменты)
  - [x] 5.2 `data/golden/transcript-{1..7}.json` + `f1-reference-{1..7}.json` + `f4-reference-{1..7}.json` (все 7 имеют F4)
  - [x] 5.3 `data/golden/manifest.json` — метаданные + статистика
  - [x] 5.4 Семантические проверки в manifest: commitments_not_empty, okr_references_not_empty, f4_three_items, f1_format_three_sections

- [x] Задача 6: Документация результатов
  - [x] 6.1 `docs/prompt-testing-results.md` — финальный вердикт GO + AC + сводки + выводы для Epic 1
  - [x] 6.2 `prompts/CHANGELOG.md` v1.0.0 (validated 2026-04-20)
